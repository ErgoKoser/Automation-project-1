...
describe('This is first test suite, Ergo Koser', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        //25.04.2024 at 11.14 a clock
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('ErgoTest')
        cy.get('#lastName').type('KoserTest')
        cy.get('input[name="password"]').type('ABC302010')
        cy.get('[name="confirm"]').type('ABC302010')
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('ABC302010')
        cy.get('[name="confirm"]').type('ABC302010ABC')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('#username').type('EKKS')
        cy.get('[data-testid="phoneNumberTestId"]').type('10203040')
        cy.get("input[name='password']").type('ABC302010')
        cy.get('[name="confirm"]').type('ABC302010')

        // Scroll back to phone nuber input field
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#firstName').type('ErgoTest')
        cy.get('#lastName').type('KoserTest')
        cy.get('[data-testid="phoneNumberTestId"]').type('Cerebrum_hub')
        cy.get("input[name='password']").type('ABC302010')
        cy.get('[name="confirm"]').type('ABC302010')

        // Assert that submit button in disabled
        cy.get('#success_message').should('not.be.visible')
